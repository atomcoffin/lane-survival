...
- **Capture/export:** the canvas keeps 1080×1920 logical coords but renders into a `CAPSCALE`× backing store (default 2×) for supersampling; `captureStream` records that. **Record** → in-browser MediaRecorder `.webm` (VP9/opus, bitrate scales with `CAPSCALE`). The maintainer converts the `.webm` to `.mp4` in **CapCut** (native/GPU encoder — far lighter than in-browser transcoding). In-browser ffmpeg.wasm export was tried and **removed**: single-thread wasm x264 was too CPU-heavy. Don't re-add an in-app transcoder without hardware acceleration (i.e. not wasm).
