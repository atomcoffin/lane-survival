...
## Files
- `index.html` — the entire app. One big inline `<script>` + `<style>`. No build step. CDN dependencies: Google Fonts (always), and **ffmpeg.wasm** (`@ffmpeg/ffmpeg` + `@ffmpeg/util` + single-thread `@ffmpeg/core`, lazy-loaded from unpkg only when the user clicks **Export MP4**). The ST core is deliberate so it works on GitHub Pages without cross-origin isolation (COOP/COEP). Keep the *engine* single-file unless asked to modularize.
- **Capture/export:** the canvas keeps 1080×1920 logical coords but renders into a `CAPSCALE`× backing store (default 2×) for supersampling; `captureStream` records that. **Record** → in-browser MediaRecorder `.webm` (VP9/opus, bitrate scales with `CAPSCALE`). **Export MP4** transcodes that webm via ffmpeg.wasm, downscaling to a crisp 1080×1920 H.264/AAC `.mp4` (TikTok's delivery format; in-browser 4K x264 is too slow, so 1080 is the deliberate target). If ffmpeg fails to load (offline/blocked), the `.webm` + CapCut path still works.
