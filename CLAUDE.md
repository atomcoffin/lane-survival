...
- **Capture/export:** the canvas keeps 1080×1920 logical coords but can render into a larger backing store for supersampling, controlled by `CAPSCALE` (active scale) via `applyCaptureScale()` (sets `cv.width/height` + a base `setTransform`). **Supersampling is expensive, so live/preview stays at `CAPSCALE=1`; it only switches up to `recScale` (the "Recording quality" dropdown, 1×/2×/3×, default 1×) for the duration of a recording**, then reverts on stop. The cached dot-grid (`gridBg`) is baked at device resolution and blitted 1:1 (`gridScale` guards rebuilds on scale change). **Record** → in-browser MediaRecorder `.webm` (VP9/opus, bitrate ~16 Mbps × `CAPSCALE`). The maintainer converts the `.webm` to `.mp4` in **CapCut** (native/GPU encoder — far lighter than in-browser transcoding). In-browser ffmpeg.wasm export was tried and **removed**: single-thread wasm x264 was too CPU-heavy. Don't re-add an in-app transcoder without hardware acceleration (i.e. not wasm).
